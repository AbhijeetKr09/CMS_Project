...
const ArticleEditor = () => {
    ...
    // Publish a staged article (editor/admin only)
    const handlePublish = async () => {
        if (!title.trim()) {
            setStatus('Title is required before publishing.');
            setStatusType('error');
            return;
        }
        showConfirm(
            'Publish Article',
            'Are you sure you want to publish this article? It will go live immediately.',
            'Publish Now',
            'Cancel',
            async () => {
                closeConfirm();
                ...
                try {
                    ...
                    await api.put(`/cms/staged/${id}`, payload);
                    await api.post(`/cms/staged/${id}/publish`);
                    setStagedArticleStatus('PUBLISHED');
                    setStatus('Article published ✓');
                    setStatusType('success');
                } catch (err) {
                    console.error(err);
                    setStatus(err.response?.data?.message || 'Error publishing article');
                    setStatusType('error');
                } finally {
                    setSaving(false);
                }
            ...
            <header className="sticky top-0 z-40 bg-bg-secondary/80 backdrop-blur-xl border-b border-border">
                <div className="max-w-screen-2xl mx-auto px-6 flex items-center justify-between h-14">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isEditorOrAdmin && stagedArticleStatus === 'SUBMITTED' ? (
                            // Editor view on a SUBMITTED article: Save changes + Publish
                            <>
                                <button
                                ...
                                >
                                    {saving ? (
                                        ...
                                        <><HiOutlineCloudUpload className="w-4 h-4" />Save</>
                                    )}
                                </button>
                                ...
                                <button
                                    ...
                                    disabled={saving}
                                    className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white px-5 py-2 rounded-full font-semibold text-sm transition-all shadow-lg shadow-green-600/20 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {saving ? (
                                        <><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Publishing...</>
                                    ) : (
                                        <><HiOutlinePaperAirplane className="w-4 h-4" />Publish Article</>
                                    )}
                                </button>
                            </>
                        ) : (
                            // Journalist / other states: Save Draft + Submit for Review
                            <>
                                <button
                                    onClick={handleSaveDraft}
